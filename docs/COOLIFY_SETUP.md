# Deploying KynexEdu ERP on Coolify (v4.x) — Setup Guide
...
This guide assumes the portal is hosted at **`edu.kynexsolutions.com`**, with each
school at **`{slug}.edu.kynexsolutions.com`**. Your apex `kynexsolutions.com` (the
marketing site) is a **separate** app and is left untouched throughout.
...
- **Wildcard DNS**: `*.edu.kynexsolutions.com → VPS IP` (Part 1).
...
> listed in [`config/tenancy.php`](../config/tenancy.php) → `central_domains`**. It
> is already set to `edu.kynexsolutions.com` (plus `localhost` for dev). If you ever
> deploy on a different host, **add it there, commit, and redeploy** — otherwise the
> portal host is mistaken for a tenant slug and every page 404s with "School not
> found." See [§1.3](#13-confirm-your-domain-is-a-central-domain-in-code).
...
> 💡 **Run it next to your other Coolify apps (e.g. AxioRCM) — that's fine.** Put
> KynexEdu in its **own Project** for tidiness, give it its **own database** (the
> bundled Compose stack does this), and Traefik keeps everything separate by
> hostname. A 32 GB VPS comfortably runs both. Do **not** share another app's
> Postgres — KynexEdu creates a database per tenant and needs an isolated DB role.
...
record isn't enough. At your DNS provider (where `kynexsolutions.com` is managed):

| Action | Type | Name | Value | Purpose |
|---|---|---|---|---|
| **add** | `A` | `edu` | *your VPS IP* | the portal `edu.kynexsolutions.com` |
| **add** | `A` | `*.edu` | *your VPS IP* | every school `*.edu.kynexsolutions.com` |
| **leave** | — | `@` / `www` | *(your marketing site)* | **don't touch** — the apex keeps serving `kynexsolutions.com` |

The wildcard `*.edu` only matches one label below `edu`, so it can't shadow your
apex or other subdomains. Verify before continuing:
...
must contain the host you serve the **portal** on — and nothing belonging to
another app. It is already scoped correctly for this deployment:

```php
'central_domains' => [
    '127.0.0.1',
    'localhost',
    'edu.kynexsolutions.com',   // ← the KynexEdu portal host
],
```

- `edu.kynexsolutions.com` — the KynexEdu portal. ✅ already set.
- `kynexsolutions.com` (apex) — your **separate marketing site**, deliberately
  **not** listed here, so KynexEdu never claims it.
- `localhost` / `127.0.0.1` — local development.

Why it matters: the tenant-resolution middleware
([`InitializeTenancyBySubdomainOrDomain`](../app/Http/Middleware/InitializeTenancyBySubdomainOrDomain.php))
treats a host *in* this list as the **central portal**, and a host *ending in*
`.{central}` as a **school slug**. So `edu.kynexsolutions.com` → portal, and
`demo.edu.kynexsolutions.com` → tenant `demo`. If you ever serve the portal on a
different host, add it here, commit, and redeploy — otherwise the portal itself is
read as a (non-existent) tenant and 404s.
...
3. Connect the repo (`git@github.com:SMQ05/Kynex_edu_EPR.git`):
   - **Private Repository (GitHub App)** if you connected one, or
   - **Public Repository** with the URL.
...
1. Create a scoped DNS API token at your provider (Cloudflare: a token with
   **Zone → DNS → Edit** on the `kynexsolutions.com` zone).
...
- [ ] `central_domains` in `config/tenancy.php` = `edu.kynexsolutions.com` (committed)
- [ ] DNS: `A edu → VPS IP` **and** `A *.edu → VPS IP` (both resolve via `dig`); apex/`www` left on the marketing site
...
Your portal host isn't in `central_domains` (so it's read as a tenant slug). Confirm
[`config/tenancy.php`](../config/tenancy.php) lists `edu.kynexsolutions.com`, commit,
redeploy. See [§1.3](#13-confirm-your-domain-is-a-central-domain-in-code).
...
- **Sizing:** 2 GB / 1 vCPU runs the portal + a few tenants; the image bundles
  ...
  audits or host many active schools. (On a 32 GB VPS this runs comfortably
  alongside other Coolify apps.)
