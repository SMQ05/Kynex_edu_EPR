...
class SchoolPortalController extends Controller
{
    ...
    public function setPassword(Request $request, string $token): RedirectResponse|View
    {
        $request->validate([
        ...
        if ($invitation->type === 'school_signup') {
            // Self-signup goes to a Signup Lead in pending state. SaaS
            // admin reviews and approves before any tenant DB is
            // provisioned. The applicant's chosen password is stashed
            // in invitation->meta until approval.
            $this->queueSignupForSaasApproval($invitation, $request->password);
            $invitation->update(['consumed_at' => now()]);

            return view('portal.signup-pending', ['email' => $invitation->email]);
        }
    ...
    public function sendResetLink(Request $request): RedirectResponse
    {
        ...
        // 1. Try every tenant DB for an existing SchoolUser
        $found = false;
        foreach (Tenant::all() as $tenant) {
            try {
                tenancy()->initialize($tenant);
                ...
                Log::warning('forgot-password tenant probe failed', [
                    'tenant_id' => $tenant->id,
                    'error'     => $e->getMessage(),
                ]);
                if (tenancy()->initialized) {
        ...
        // 2. Not in any tenant? Maybe the user registered but never finished
        //    set-password (or hit the prior 500). Look for a recent signup
        //    invitation and reissue a fresh set-password link so they can
        //    complete their registration.
        if (! $found) {
            $invitation = SchoolInvitation::where('email', $email)
                ->where('type', 'school_signup')
            ...
            if ($invitation) {
                ...
                try {
                    app(\Resend\Client::class)->emails->send([
                        'from'    => 'KynexEdu <noreply@kynexsolutions.com>',
                        'to'      => $email,
                        'subject' => 'Continue your KynexEdu school registration',
                    ...
                    Log::error('Failed to resend signup invite via forgot-password', [
                        'email' => $email,
                        'error' => $e->getMessage(),
                    ]);
                }
        ...
        return back()->with('success', 'If an account or pending registration exists for that email, we have just emailed a fresh link. Please check your inbox (and spam folder).');
    }
    ...
    /**
     * Convert the verified self-signup invitation into a SignupLead row
     * that the SaaS admin must approve. Stashes the chosen password
     * (hashed) into the lead's meta so we can finish provisioning
     * unchanged at approval time.
     */
    private function queueSignupForSaasApproval(SchoolInvitation $invitation, string $password): void
    {
        $existing = \App\Models\TenantSignup::where('email', $invitation->email)
            ->whereNotIn('status', ['rejected', 'onboarded'])
            ->first();
        ...
        $payload = [
            ...
            'email'              => $invitation->email,
            'phone'              => $invitation->phone ?? '',
            'whatsapp_number'    => $invitation->meta['whatsapp_number'] ?? null,
            'city'               => $invitation->meta['city'] ?? null,
            'country'            => $invitation->meta['country'] ?? 'Pakistan',
            'message'            => $invitation->meta['message'] ?? null,
            'status'             => 'new',
            'internal_notes'     => 'Awaiting SaaS approval to provision tenant DB.',
        ];
        ...
        if ($existing) {
            ...
            $lead = \App\Models\TenantSignup::create($payload);
        }

        // Stash the password hash on the invitation so the approval
        // handler can finish provisioning without asking the applicant
        // to set a password again.
        $invitation->update([
            'meta' => array_merge((array) $invitation->meta, [
