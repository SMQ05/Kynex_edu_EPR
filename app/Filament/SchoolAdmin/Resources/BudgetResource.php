            ...
            ->components([
                Section::make('Budget Details')
                    ->schema([
                        ...
                        TextInput::make('budgeted_amount_pkr')
                            ->label('Budgeted Amount (PKR)')
                            ->numeric()
                            ->required()
                            ->prefix('PKR')
                            ->dehydrateStateUsing(fn ($state) => (int) (($state ?? 0) * 100))
                            ->formatStateUsing(fn ($state, $record) => $record ? $record->budgeted_amount_paisas / 100 : 0)
                            ->dehydrated(false)
                            ->afterStateHydrated(function (TextInput $component, $record) {
                                if ($record) {
                                    $component->state($record->budgeted_amount_paisas / 100);
                                }
                            }),
