...
class CampusResource extends Resource
{
    ...
    /**
     * Campus creation is reserved for institute-level governance. School
     * admins operate within their own campus only — they can view but not
     * spawn new campuses.
     */
    public static function canCreate(): bool
    {
        return self::userHasInstitutePrivilege();
    }
    ...
    protected static function userHasInstitutePrivilege(): bool
    {
        $user = auth()->guard('school_users')->user();
        if (! $user) {
            return false;
        }
        // Active-role-aware: campuses can be created by SaaS admin (via
        // the cross-tenant SaaS panel) and on the school side by
        // MULTI_INSTITUTE_HEAD or INSTITUTE_HEAD acting in that role.
        // School admins / receptionists / teachers stay locked out.
        $activeRole = $user->active_role ?? $user->roles->first()?->name;
        return in_array($activeRole, ['MULTI_INSTITUTE_HEAD', 'INSTITUTE_HEAD'], true);
    }
}
