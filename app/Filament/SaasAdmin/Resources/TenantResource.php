        ...
        return $schema->schema([
            ...
            Section::make('AI Settings')
                ...
                ->schema([
                    ...
                    Components\Select::make('ai_provider')
                        ->label('AI Provider')
                        ->options([
                            'openrouter' => 'OpenRouter (multi-model, best for variety)',
                            'groq'       => 'Groq (fast inference, lower cost)',
                        ])
                        ->default('openrouter')
                        ->live()
                    ...
                    Components\TextInput::make('ai_openrouter_api_key')
                        ->label(fn (Get $get): string => match ($get('ai_provider')) {
                            'groq'       => 'Groq API Key (per-school override)',
                            default      => 'OpenRouter API Key (per-school override)',
                        })
                        ->password()
                        ->revealable()
                        ->placeholder(fn (Get $get): string => match ($get('ai_provider')) {
                            'groq'       => 'gsk_xxxxxxxxxxxxxxxx',
                            default      => 'sk-or-v1-xxxxxxxxxxxx',
                        })
                        ->helperText('Per-school API key. Get Groq keys at console.groq.com — get OpenRouter keys at openrouter.ai/keys. Leave blank to use the platform-wide key from API Settings.')
                        ->visible(fn (Get $get): bool => (bool) $get('ai_enabled')),
