 ...
 *
 * Persists auth token in expo-secure-store and manages user state.
...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  login: async (email: string, password: string, schoolSlug: string) => {
    ...
    try {
      // Set the tenant-specific API base URL
  ...
  checkAuth: async () => {
    ...
    try {
      ...
      // Verify token by attempting a refresh
  ...
  switchRole: async (role: string) => {
    try {
      ...
      if (user) {
        ...
        // Update active_role locally if API doesn't return full user
